...
def run_preprocessing():
    """Preprocess all segments in data/filtered_segments/ (or data/segments/) → data/processed/."""
    settings = load_settings()
    paths = get_data_paths(settings)
    target_sr = settings.get("audio", {}).get("sample_rate", 16000)

    # Detect if audience filtering has been run (use data/filtered_segments if populated)
    project_root = paths["raw_audio"].parent.parent
    filtered_dir = project_root / "data" / "filtered_segments"
    
    if filtered_dir.exists() and any(filtered_dir.glob("*.wav")):
        seg_dir = filtered_dir
        logger.info("Using filtered segments (audience-reaction free) from: %s", filtered_dir)
    else:
        seg_dir = paths["segments"]
        logger.warning("data/filtered_segments/ is empty or does not exist. Falling back to unfiltered segments from: %s", seg_dir)
